...
showFundScreen = function(fundId,name) {
	...
	$('html, body').animate({
        scrollTop: $("#fundScreenBody").offset().top
				...
				fundsChartSingleArray = [];
//console.log('fundsChartSingleArray ' +fundsChartSingleArray);
				fundsChartSingleArray.push(fundId+';'+name);
//console.log('fundsChartSingleArray ' +fundsChartSingleArray);
	...
	$.each(data[0], function(key, val) {
		$.each(val, function(index, vale) {
			thePriceDate = vale.priceDate;
			console.log(data);
			
			if (fundId == vale.id)
			{
				var riskClass = 'circle-risk rated-'+vale.risk+'';
				$('#fundScreenBody #fundRisk').addClass(riskClass);
				$("#fundScreenBody #fundRisk").html('IL-'+vale.risk);
				$("#fundScreenBody #price").html(vale.latest);
				$("#fundScreenBody #date").html(vale.priceDate);
				$('#factsheet').attr('href',vale.url);
				$("#factsheet").attr('onclick','dataLayer.push({\'event\':\'trackEvent\',\'category\':\'website\',\'action\':\'Download PDF\',\'label\':'+vale.name+',\'value\':\'1\'});return true');
				$("#fundScreenBody #fundName").html("<span id=\"theName\">"+vale.name+"</span><i onclick=\"hideFund();\" class=\"material-icons hideChartClose\" style=\"display: block;\">close</i>");
				$('#fundScreenBody #lastMonth').html(vale.lastMonthGrowth);
				$('#fundScreenBody #sixMonth').html(vale.sixMonthGrowth);
				$('#fundScreenBody #1year').html(vale.last12Months);
				$('#fundScreenBody #3years').html(vale.last3Years);
				$('#fundScreenBody #3yearsAnn').html(vale.last3YearsAnn);
				$('#fundScreenBody #5yearsAnn').html(vale.last5yearsAnn);
				$('#fundScreenBody #sinceLaunch').html(vale.sinceLaunchGrowth);
				
				if($('#theName').html() == "Protected Consensus Markets") {
					$( ".addFundGuide" ).after("<div class=\"row-item fundGuide\"><div class=\"col-lg-2 col-md-2 col-sm-2 col-xs-2\"><img src=\"https://www.irishlife.ie/myonlineservices/Content/images/icons/pdf_icon_grey.png\" style=\"width: 48px; height: 48px\"></div><div class=\"col-lg-10 col-md-10 col-sm-10 col-xs-10 \"><span style='font-size:11px;'><span style=\"display:none\">Protected Price Pledge: &euro;"+vale.protPrPl+"<br/>Exposure to Consensus Markets Fund: "+vale.expsConsMktFnd+"</span><a href=\"https://www.irishlife.ie/sites/retail/files/platform-factsheets/Retail-Factsheets/pcmf-fund-guide.pdf\" target=\"_blank\">Protected Consensus Markets Fund Guide</a></span></div></div>");
				}
				else {
					$('.fundGuide').hide();
				}
				
				if($('#theName').html() == "Indexed Technology") {
					$('.addFundGuide').hide();
				}
				else {
					$('.addFundGuide').show();
					...
					*/
					for (x=0;x < vale.calPerf.length;x++)
					{
						...
						performanceData.push(parseFloat(strPerfArr[1]) * 100);
	...
	getChartDataAgain('single');
	$('#fundScreenBody').show();
	// set the url hash
	//
	$('#fundScreenBody').html($('#fundScreenBody').html() + fundContent);
	

				
	// new data function here for yearly performance chart - call initFundPerformanceChart from that function		
				
	initFundPerformanceChart('fundPerfChart', fundName, performanceData, performanceCategories);
...
getChartDataAgain = function(whichChart){
	...
	$.each(fundsSelected, function(z, name) {
		...
		if(this.length>=3 & fundsSelected[z].split(';')[0] !='chartscreentrue'){ // fund names must be 3 or 4 chars
			$.getJSON('https://www.irishlife.ie/servlet/fundpricejson?fund='+fundsSelected[z].split(';')[0], function (data, error){
			...
			.always(function ( dataORjqXHR, textStatus, jqXHRORerrorThrown) {
				...
				"</h2><img alt=\"Loading\" src=\"/sites/all/modules/fund_centre/css/images/ajax-loader.gif\"><h5>"
...
pullDownOverallFundData = function(){
	...
	{
		var jqxhr = $.get( "https://www.irishlife.ie/servlet/jsonFund.jsp?group="+fundGroupMain+"&page=prices", function() {
		})
		.done(function(data) {
	...
	$.each(data[0], function(key, val) {
		$.each(val, function(index, vale) {
			...
			tableBodyContents[fundGroupIDtemp] += "<td style=\"text-align:left\" class=\"name\"><a " +
				"onclick=\"showFundScreen('"+vale.id+"','"+vale.name+"');\"" + "href=\"#\"><span class=\"fundName\">"+vale.name+"</span></a>";

			tableBodyContents[fundGroupIDtemp] += "<td class=\"mobileRow\"><div class=\"mobileRiskRating circle-risk rated-"+vale.risk+"\"><span class=\"rateditem\">IL-"+vale.risk+"</span></div><div class=\"mobilePrice "+ fc(vale.latest) +" \">&euro;"+vale.latest+"</div><div class=\"mobileFromLaunch "+ fc(vale.sinceLaunchGrowth) +" \">"+vale.sinceLaunchGrowth+"<div></td>";
			...
			/*if (vale.url.length<=10){
				...
				tableBodyContents[fundGroupIDtemp] += "<td class=\"name leftAlign\"><a " +
				"onclick=\"dataLayer.push({'event':'trackEvent','category':'Fund Factsheet','action':'Download PDF','label': '"+vale.name+"','value':'1'});return true\" " +
				
				" href=\""+vale.url+"\" download=\""+vale.name+" - "+vale.id+".pdf\"><span class=\"pdf\"></span><span class=\"fundName\">"+vale.name+"</span></a>";
			}*/
			
			//tableBodyContents[fundGroupIDtemp] += "<td class=\"risk\" "+Drupal.settings.fund_centre.riskDisplay+ "><div class=\"circle-risk rated-"+vale.risk+"\">\
			//<span class=\"rateditem\">"+vale.risk+"</span></div></td>";

			/*if (vale.protPrPl > 0) {
				tableBodyContents[fundGroupIDtemp] += "<br/><span style='font-size:11px;'><span style=\"display:none\">Protected Price Pledge: &euro;"+vale.protPrPl+"<br/>Exposure to Consensus Markets Fund: "+vale.expsConsMktFnd+"</span><a href=\"https://www.irishlife.ie/sites/retail/files/platform-factsheets/Retail-Factsheets/pcmf-fund-guide.pdf\" target=\"_blank\">Protected Consensus Markets Fund Guide</a></span></td>";
			} else{
				tableBodyContents[fundGroupIDtemp] += "</td>";
			} */
			
			tableBodyContents[fundGroupIDtemp] += "<td class=\"charge\" "+Drupal.settings.fund_centre.chargeDisplay+">"+vale.fmc+"%</td>\
			<td style=\"display:none\" class=\"1mth "+fc(vale.lastMonthGrowth)+"\">"+vale.lastMonthGrowth+"</td>\
			<td class=\"whenCharted 3mth "+fc(vale.threeMonthGrowth)+"\">"+vale.threeMonthGrowth+"</td>\
			<td class=\"whenCharted 6mth "+fc(vale.sixMonthGrowth)+"\">"+vale.sixMonthGrowth+"</td>\
			<td class=\"whenCharted 12mth "+fc(vale.last12Months)+"\">"+vale.last12Months+"</td>\
			<td class=\"whenCharted 3yrs "+fc(vale.last3Years)+"\">"+vale.last3Years+"</td>\
			<td class=\"whenCharted 5yrs "+fc(vale.last5years)+"\">"+vale.last5years+"</td>\
			<td style=\"display:none\" class=\"10yrs "+fc(vale.last10Years)+"\">"+vale.last10Years+"</td>\
			...
			<td class=\"whenCharted fromlaunch "+fc(vale.sinceLaunchGrowth)+"\">"+vale.sinceLaunchGrowth+"</td>\
			<td class=\"risk\" "+Drupal.settings.fund_centre.riskDisplay+ "><div class=\"circle-risk rated-"+vale.risk+"\">\
			<span class=\"rateditem\">IL-"+vale.risk+"</span></div></td>\
...
getProductFundPrices = function(id, name){
	...
	else
	{
		if (id > 0)
		{
			var jqxhr = $.get( "https://www.irishlife.ie/servlet/jsonFund.jsp?group="+id+"&page=prices ", function() {
			})
			.done(function(data) {
				var myJSON = $.parseJSON(data.replace(/&#034;/g,'\"'));
	...
	*/
	...
	$.each(data[0], function(key, val) {
		$.each(val, function(index, vale) {
			...
			if (vale.url.length<=10){
				localTableHolder[fundGroupIDtemp] += "<td style=\"text-align:left\" class=\"name\"><span class=\"fundName\">"+vale.name+"</span></td>";
			} else{
				localTableHolder[fundGroupIDtemp] += "<td style=\"text-align:left\" class=\"name\"><span class=\"fundName\">"+vale.name+"</span></td>";
				/*
				This code is to add in the download link for the fund. remove line above if uncommenting
			...
			localTableHolder[fundGroupIDtemp] += "<td class=\"mobileRow\"><div class=\"mobilePrice "+ fc(vale.latest) +" \">&euro;"+vale.latest+"</div><div class=\"mobileFromLaunch "+ fc(vale.sinceLaunchGrowth) +" \">"+vale.sinceLaunchGrowth+"<div></td>";
			...
			if ($.inArray(vale.id, fundsChartStoredArrayForProducts) >= 0){
				localTableHolder[fundGroupIDtemp] += "<input id=\"charted-"+vale.id+"\" class=\"charted-input\" type=\"checkbox\" checked/><label class=\"feaeaea chart-on\" for=\"charted-"+vale.id+"\"></label><span class=\"chartAgain\" style=\"display:none\">1</span></td></td></tr>";
			}
			else{
				localTableHolder[fundGroupIDtemp] += "<input id=\"charted-"+vale.id+"\" class=\"charted-input\" type=\"checkbox\" /><label class=\"feaeaea\" for=\"charted-"+vale.id+"\"></label><span class=\"chartAgain\" style=\"display:none\">0</span></td></td></tr>";
			}
...
getFundProductsList = function(){
	...
	$.get('https://www.irishlife.ie/servlet/jsonFund.jsp?group='+productGroupsId+'&page=groups', function (data, error){
