    ...
    (function() {
        ...
        var logGaEvent = function(gaEvent) {
    		...
    		try{
    			...
    			dataLayer.push({
    			'event':'trackEvent',
    			'category':myILFS.quoteType+"-"+gaEvent.category,
...
function drawCostSumChart(prem,lifeCover) {
	prem = prem*12;
	prem = prem.toFixed(0);
	var premArray = [];
	var lifeCoverArray = [];
	var yearsToBreakEven = (lifeCover/prem).toFixed(0);
	var years= [];
	for (i=1;i<=35;i++)
	{
		premArray.push(prem*i);
		lifeCoverArray.push(Number(lifeCover));
		years.push(i);
	}
$('#costVSumChart').highcharts({
        title: {
            text: 'Cost of Life Cover vs €'+lifeCover + ' Sum Assured',
            x: -20 //center
        },
        credits:false,
        subtitle: {
            text: 'This chart compares the cumulative pemiums paid over time with the amount of life cover sought.<br>'+
			'It will take <b>' + yearsToBreakEven + ' years </b> to break even',
            x: -20
        },
        xAxis: {
        title: {
        ...
        series: [{
            name: 'Total Premiums Paid over time<br>Annual Premium is €' + prem,
            data: premArray
        },
		{name: 'Life cover sought',
            data: lifeCoverArray
        }
		]
    });
...
function drawRateReturnChart(prem,lifeCover,youngestAge) {
	prem = prem*12;
	prem = prem.toFixed(0);
	var premArray = [];
	var rateReturnArray = [];
	var yearsFromToday = [];
	var numItemsInArray = 0;
	for (i=1;i<=200;i++)
	{
		premArray.push(prem*i);
	}
	
	for (i=0;i<200 && numItemsInArray <= 20;i++)
	{
		var ror = ((lifeCover-premArray[i])/premArray[i])*100;
		ror = ror.toFixed(0);
		if (ror <= 200)
		{			
			rateReturnArray.push(Number(ror));
			yearsFromToday.push(youngestAge+i+1);
			numItemsInArray++;
		}
	}
...
$('#rateReturnChart').highcharts({
        title: {
            text: 'Potential Rate of Future Return',
            x: -20 //center
        },
        credits:false,
        subtitle: {
            text: 'This chart shows the rate of return for each year of the plan term if a death claim is made in that year.',
            x: -20
        },
        xAxis: {
        title: {
                text: 'Youngest Client Age - currently <b>' + youngestAge + ' years old</b>.'
            },
            categories: yearsFromToday
        },
        yAxis: {
            title: {
                text: '%Rate Return'
        ...
        series: [{
            name: 'Rate of Return for <br> Death Claim at this age',
            data: rateReturnArray
        }]
    });
}
...
myILFS._callbackRHS = function(callback){
    myILFS.callbackRHS = callback;
}
...
myILFS._setGAPageViewQuoteStarted = function(){
	myILFS._setGAPageView('/life-assurance/quote-started','Quote started');
}

myILFS._setGAPageViewQuoteCompleted = function(){
	myILFS._setGAPageView('/life-assurance/quote-completed','Quote completed');
}
...
//	stylesheet.href = 'http://ildev.internal.irishlife.ie:8080/uploadedFiles/retail/123/css/ilfsWidget.css';
	...
	'<div id="setColB" class="stepCol"><span class="step">Step</span> 2&nbsp; <span class="title">-&nbsp;Your Options</span><span class="ilfsArrowRight"></span> </div>'+
	'<div id="setColC" class="stepCol last"><span class="step">Step</span> 3&nbsp; <span class="title">-&nbsp;Call Me Back</span></div><div class="clear"></div></div></div>';

	if (isBlineWholeLife)
	{
		html.progress = '<div class="row"><div class="twelvecol stepHold">'+
		'<div id="setColA"  style="width:49%" class="stepCol first stepActive"><span class="step">Step</span>&nbsp;1&nbsp;<span class="title">-&nbsp;Your Details</span><span class="ilfsArrowRight" ></span></div>'+
		'<div id="setColB" style="width:50%" class="stepCol last"><span class="step">Step</span> 2&nbsp; <span class="title">-&nbsp;View Quote and Cost</span></div><div class="clear"></div></div></div>';
	}
	
	if(premURL){
		html.sliderHeader = '<h2 class="ilfsTitle">Slide To Set The Desired Monthly Payment</h2>';
	}
	else {
	...
	html.sliderBox = '<div class="ilfsFormColumn"><div class="row smallRow"><div class="eightcol ilfsDetailsPanel"><fieldset><div class="ilfsSliderTextPanel" style="display:none;"></div>';

    if(premURL){
        html.sliderBox += '<div id="ilfsPremiumQuoteSlider"></div></fieldset></div><div class="fourcol ilfsDetailsPanel last sliderResultBlock ' + (ILFSProductTerm ? '' : 'termFalse') + '">€<span id="quoteResult" >15</span>' + (ILFSProductTerm ? html.termHolder : '' ) + '</div></div><div class="clearer"></div></div>';
    } else {
         html.sliderBox += '<div id="ilfsQuoteSlider"></div></fieldset></div><div class="fourcol ilfsDetailsPanel last sliderResultBlock ' + (ILFSProductTerm ? '' : 'termFalse') + '">€<span id="quoteResult" >100,000</span>' + (ILFSProductTerm ? html.termHolder : '' ) + '</div></div><div class="clearer"></div></div>';
    }
	...
	html.screen1ComplyBox = '<strong>This indicative quotation is for a '+ILFSProductScreenProductName+' protection policy which is provided by Irish Life Assurance plc.</strong> You may have other protection needs and we always recommend that you should discuss your overall insurance needs with a Financial Advisor.</div>';
	html.screen2ComplyBox = '<tr><td class="content ilfsFormColumn ilfsComply quoteDetailsInfo"><strong>This indicative quotation is for a '+ILFSProductScreenProductName+' protection policy which is provided by Irish Life Assurance plc. and is subject to underwriting.</strong> It can provide a cash sum in the event of death (life cover). No cash will be returned if you cancel your plan or stop making regular payments on your plan during the term. If this happens you will no longer be covered.</td></tr><tr><td class="content ilfsFormColumn ilfsComply quoteDetailsInfo">The quote is valid either for 7 days or to your next birthday, whichever is sooner. This is subject to no changes to the regulatory or tax rules applying to the product within that period.</td></tr><tr><td class="content ilfsFormColumn ilfsComply quoteDetailsInfo">For the online quote we assume that your contributions do not increase in line with inflation and that you are not choosing the Guaranteed Cover Again option. You can update these over the phone to receive a different quote.</td></tr>';
	...
	html.screen4ComplyBox = '<div class="ilfsFormColumn ilfsComply">In the interest of customer service we will record and monitor calls. We will only use your personal details for the purpose of dealing with your request. Your privacy is important to us. By completing your contact details above, you authorise us to contact you by telephone, mobile and/or email.</div>';

	// the second screen
	html.detailsNext2 = '<h2 class="ilfsTitle">'+(isBlineWholeLife?'The Quote':'Your Quote')+'</h2><div class="ilfsFormColumn"><table><thead><th>'+ILFSProductScreenTitle+' Quote</th></thead><tbody><tr><td class="content"><span class="ilfsQuoteResult">€15.00</span> / month<br><span class="tableBreakdownInfo"><a class="quoteBreakdownButton" href="javascript:void(0);"  ga-click ga-category="ilfs-life-quote" ga-action="Page 2" ga-label="Show/Hide Breakdown Button Clicked"><span style="DISPLAY: inline" class="showQuoteBreakdown">Show Breakdown</span><span style="DISPLAY: none" class="hideQuoteBreakdown">Hide Breakdown</span></a></span><div style="DISPLAY: none" id="quoteBreakdownDetails" class="tableBreakdownInfo"><span class="quoteBreakdownDetails1 quoteBreakdownDetailsText">Cost of Life Cover: €8.35</span><br /><span class="quoteBreakdownDetails2 quoteBreakdownDetailsText">Irish Life Policy Fee: €6.50</span><br /><span class="quoteBreakdownDetails3 quoteBreakdownDetailsText">Government Levy: €0.15</span><br /><span class="quoteBreakdownDetails4 quoteBreakdownDetailsText">&nbsp;</span></div></td></tr><tr class="ilfsQuoteBenefits"><td>'+
	'<ul><li>Life assurance valued at <span class="progressLifeCoverAmount">€</span></li>'+
	'<li>Premium you pay is <span class="progressQuote">€</span> per month <span class="progressLifeCoverYears"></span>.</li>'+
	'<li>A <span class="progressAge1Person"></span><span class="progressSmoker1Person"></span></li>'+
	'<li class="ilfsHideSecondPersonInfo" style="display:none;">A <span class="progressAge2Person"></span><span class="progressSmoker2Person"></span></li>'+'</td></tr>'+
'<tr>\
    <td id ="emailToSelfContent" class="content" '+(isBlineWholeLife?' style="display:none" ':' ') + '>\
        <div id="emailToSelfDetails" class="tableBreakdownInfo" style="display: none;">\
            <div class="row smallRow">\
                <div class="threecol"><label>Email:</label></div>\
                <div class="sevencol last">\
                    <input id="emailToSelfmail" name="emailToSelfField" ga-focus="" ga-category="ilfs-life-quote" ga-action="Page 2" ga-label="Callback Email Field clicked">\
                    <div class="emailToSelfMailError errorMessage"></div>\
                    <div class="emailToSelfMailSuccess successMessage"></div>\
                </div> \
                <div class="clearer"></div>\
            </div>\
            <div class="row smallRow"><ul><li>By Submitting your details you agree that Irish Life may use your personal data to contact you by email, with information and marketing material about our products and services within the Irish Life Group of companies. You can opt out by clicking the unsubscribe link in any email.</li></ul></div>\
        </div>\
        <span class="tableBreakdownInfo">\
            <a class="emailToSelfButton ilfsMovebtn" href="javascript:void(0);" ga-click="" ga-category="ilfs-life-quote" ga-action="Page 2" ga-label="Show/Hide Breakdown Button Clicked">\
                <span style="display: inline;" class="showEmailToSelfBreakdown">Email me my Quote</span>\
                <span style="display: none;" class="sendQuoteEmail">Send Email</span>\
            </a>\
        </span>\
    </td>\
</tr>'+
        '</tbody></table>'+
		'<table class="quoteDetailsInfo"><tbody>'+html.screen2ComplyBox+'</tbody></table>'+
		
		
		'</div>';
	
	html.detailsNext2Btn = '<span class="loading" style="display:none;">loading...</span><a id="editQuoteBtn" class="ilfsMovebtn leftbtn" href="javascript:void(0);" ga-click ga-category="ilfs-life-quote" ga-action="Page 2" ga-label="Edit Details Button Clicked">&lsaquo;&nbsp;<span class="ilfs-hide-for-small">EDIT DETAILS</span></a><div class="clearer"></div>';
	
	html.callbackForm = '<div style="width:80%;text-align:right;"><div class="ilfsDetailsBlock"><div class="row smallRow"><div class="fivecol"><label>Name:</label></div><div class="sixcol last "><input id="callbackPopupName" name="callbackPopupName" ga-focus ga-category="ilfs-life-quote" ga-action="Page 3" ga-label="Callback Name Field clicked"><div class="clearer"></div><div class="callbackPopupNameError errorMessage" style="display:none;"></div></div><div class="clearer"></div></div>'+'<div class="row smallRow"><div class="fivecol"><label>Phone:</label></div><div class="sixcol last "><input id="callbackPopupPhone" name="callbackPopupPhone"  ga-focus ga-category="ilfs-life-quote" ga-action="Page 3" ga-label="Callback Phone Field clicked"><div class="clearer"></div><div class="callbackPopupPhoneError errorMessage" style="display:none;"></div></div><div class="clearer"></div></div>'+'<div class="row smallRow"><div class="fivecol"><label>Email:</label></div><div class="sixcol last "><input id="callbackPopupEmail" name="callbackPopupEmail" ga-focus ga-category="ilfs-life-quote" ga-action="Page 3" ga-label="Callback Email Field clicked"><div class="clearer"> </div><div class="callbackPopupEmailError errorMessage" display:none;"></div> </div> <div class="clearer"></div> </div>'+'<div class="row smallRow"><div class="fivecol"><label>Best Time to call:</label></div> <div class="sixcol last "> <select id="callbackPopupCalltime" name="callbackPopupcalltime" ga-change ga-category="ilfs-life-quote" ga-action="Page 3" ga-label="Best Time to call dropdown clicked"><option value="NOW">Now</option><option value="anytime" selected="selected">Anytime</option><option value="morning-before-12pm">Morning (before 12pm)</option><option value="afternoon-12pm-to-5pm">Afternoon (12pm to 5pm)</option><option value="evening-5pm-to-7.30pm">Evening (5pm to 7.30pm)</option></select></div><div class="row smallRow"><div class="sixcol last"><a id="ilfsSubmitCallback" class="ilfsMovebtn rightbtn" href="javascript:void(0);" ga-click ga-category="ilfs-life-quote" ga-action="Page 3" ga-label="Submit Callback Button Clicked">SUBMIT&nbsp;&rsaquo;</a></div></div><div class="clearer"></div></div>' +'</div><div class="clearer"></div></div>';
	
		
	html.callbackFormRHS = '<b>Your Name:</b><br/><input id="callbackRHSName" name="callbackRHSName" ga-focus ga-category="ilfs-life-quote" ga-action="RHS" ga-label="Callback Name Field clicked">TestConorTest</input><div class="callbackRHSNameError errorMessage" style="display:none;"></div>'+
	...
	// the third screen
	html.detailsNext3 = '<h2 class="ilfsTitle">Your Options - Callback</h2><div class="ilfsFormColumn"><p>Now that you\'ve completed a quote a Financial Advisor can call you back to look at all your options, answer your questions and see what protection plan may suit your life cover needs. Irish Life Financial Service is a tied agent of Irish Life Assurance plc for life and pensions.</p>'+html.callbackForm+'</div>';
	html.detailsNext3Btn = '<span class="loading" style="display:none;">loading...</span><a id="backToQuoteBtn" class="ilfsMovebtn leftbtn" href="javascript:void(0);" ga-click ga-category="ilfs-life-quote" ga-action="Page 3" ga-label="back to Quote Button Clicked">&lsaquo;&nbsp;<span class="ilfs-hide-for-small">BACK</a><div class="clearer"></div>';
    ...
    // the fourth screen, the Callbackcomplete screen
	html.detailsNext4 = '<h2 class="ilfsTitle"></h2><div class="ilfsFormColumn"><p id="ilfsCallbackRequestComplete">Completed</p></div>'+'<a id="getAnotherQuote" style="margin-top:15px;float:left" class="ilfsMovebtn leftbtn" href="javascript:void(0);" ga-click="" ga-category="ilfs-life-quote" ga-action="Page 1" ga-label="Get Another Quote Clicked" style="float: left;"><span class="ilfs-hide-for-small">GET ANOTHER QUOTE</span><span class="ilfs-show-for-small">NEW QUOTE</span>&nbsp;›</a>';



    //a\b testing screens
    html.detailsABBox1 = '<a class="ABOptionsBlock" target="_blank" href="/life-insurance/life-insurance-comparison-survey"><i class="fa fa-bar-chart fa-3x"></i><h2>Compare Benefits</h2><p>See how the National Consumer Agency rated our benefits</p> <span>Find out more > </span></a>';	
	if (myILFS.quoteType == 'M')
	{
	    html.detailsABBox1 = '<a class="ABOptionsBlock" target="_blank" href="/life-insurance/protect-my-mortgage/mortgage-protection-comparison-survey"><i class="fa fa-bar-chart fa-3x"></i><h2>Compare Benefits</h2><p>See how the National Consumer Agency rated our benefits</p> <span>Find out more > </span></a>';
	}
	else if (myILFS.quoteType == 'T')
	{
	    html.detailsABBox1 = '<a class="ABOptionsBlock" target="_blank" href="/life-insurance/protect-my-family/term-life-insurance-comparison-survey"><i class="fa fa-bar-chart fa-3x"></i><h2>Compare Benefits</h2><p>See how the National Consumer Agency rated our benefits</p> <span>Find out more > </span></a>';
	}
	else if (myILFS.quoteType == 'W')
	{
	    html.detailsABBox1 = '<a class="ABOptionsBlock" target="_blank" href="/life-insurance/protect-my-family/life-long-insurance-comparison-survey"><i class="fa fa-bar-chart fa-3x"></i><h2>Compare Benefits</h2><p>See how the National Consumer Agency rated our benefits</p> <span>Find out more > </span></a>';
	}

    html.detailsABBox2 = '<a id="callbackScreen" class="ABOptionsBlock" href="javascript:void(0);"><i class="fa fa-phone fa-3x"></i><h2>Call me back</h2><p>Discuss your options further with a qualified Financial Advisor</p><span>Find out more > </span></a>';
    html.detailsABBox3A = '<a class="ABOptionsBlock" target="_blank" href="/life-insurance/protect-my-family/buy-online/simple-life-insurance-online"><i class="fa fa-umbrella fa-3x"></i><h2>Simple Life Insurance</h2><p>Find out about our on-line Simple Life Insurance today</p><span>Find out more > </span></a>';
    html.detailsABBox3B = '<a class="ABOptionsBlock" target="_blank" href="/life-insurance/protect-your-family/buy-online/over-50s-life-insurance"><i class="fa fa-coffee fa-3x"></i><h2>Over 50?</h2><p>Apply on-line for Over 50\'s Insurance today</p><span>Find out more > </span></a>';
    html.detailsABBox3C = '<a class="ABOptionsBlock" target="_blank" href="/financial-advice/financial-planning/find-a-financial-advisor"><i class="fa fa-compass fa-3x"></i><h2>Find a local Advisor</h2><p>Find a financial advisor local to you</p><span>Find out more > </span></a>';
	html.detailsABBox3D = '<div id="FPOption1" class="large-4 columns center"><a class="ABOptionsBlock" target="_blank" href="/lifecare"><i class="fa fa-heartbeat fa-3x"></i><h4>Life Care</h4><p>Find out more about other services available when you take out a new Irish Life plan</p><span>Find out more &gt; </span></a></div>';
	html.detailsABBox3E ='<div id="FPOption2" class="large-4 columns center"><a class="ABOptionsBlock" target="_blank" href="/life-insurance/protect-my-family/ireland-s-no1-life-insurer"><i class="fa fa-smile-o fa-3x"></i><h4>We\'re #1</h4>                <p>Find out why Irish Life is Ireland\'s number 1 life Insurer</p><span>Find out more &gt; </span></a></div>';
	    
	if (isBlineWholeLife)
	{
		html.detailsAB = '<div class="ilfsFormColumn twelvecol" ><div id="costVSumChart" style="width:918px; height: 400px; margin: 0 auto">Chart goes here</div><div id="rateReturnChart" style="width: 918px; height: 400px; margin: 0 auto">Chart goes here</div></div>';
	}
	else
	{
		html.detailsAB = '\
        <div class="ilfsFormColumn" >\
            <div class="row smallRow">\
                <div class="fourcol centerText">'+html.detailsABBox1+'</div>\
                <div class="fourcol centerText">'+html.detailsABBox2+'</div>\
                <div id="detailsABOption1" style="display:none" class="fourcol centerText">'+html.detailsABBox3A+'</div>\
                <div id="detailsABOption2" style="display:none" class="fourcol centerText">'+html.detailsABBox3B+'</div>\
                <div id="detailsABOption3" style="display:none" class="fourcol centerText">'+html.detailsABBox3C+'</div>\
				<div id="detailsABOption4" style="display:none" class="fourcol centerText">'+html.detailsABBox3D+'</div>\
				<div id="detailsABOption5" style="display:none" class="fourcol centerText">'+html.detailsABBox3E+'</div>\
                <div class="clearer"></div>\
            </div>\
        </div>';
	}
    ...
    html.detailsNextABBtn = '';
    // all the screens added

    if(myILFS.callbackRHS == "true"){
        $('#ilfsWidget').html(html.progress+'<div class="row">'+
        '<div class="eightcol">'+
        '<div id="screen1">'+html.sliderHeader+html.sliderBox+html.detailsHeader+html.detailsBox+html.screen1ComplyBox+html.detailsNextBtn+'</div>'+
        '<div id="screen2" style="display:none">'+html.detailsNext2+html.detailsNext2Btn+'</div>'+
        '<div id="screen3" style="display:none">'+html.detailsNext3+html.screen4ComplyBox+html.detailsNext3Btn+'</div>'+
        '<div id="screen4" style="display:none;">'+html.detailsNext4+'</div></div>'+
        '<div class="fourcol last"><div id="ilfsRHSCallback">'+html.detailsRHSCallback+'</div></div></div>');
    } else {
        $('#ilfsWidget').html(html.progress+'<div class="row">'+
        '<div class="twelvecol">'+
        '<div id="screen1">'+html.sliderHeader+html.sliderBox+html.detailsHeader+html.detailsBox+html.screen1ComplyBox+html.detailsNextBtn+'</div>'+
        '<div id="screen2" style="display:none">'+html.detailsNext2+html.detailsAB+html.detailsNext2Btn+'</div>'+
        '<div id="screen3" style="display:none">'+html.detailsNext3+html.screen4ComplyBox+html.detailsNext3Btn+'</div>'+
        '<div id="screen4" style="display:none;">'+html.detailsNext4+'</div>');
    }


	for(i=5;i<=ILFSTermMax;i+=5) {
		$('#term').append('<option value='+i+(i==20?selectedOption:'')+' >'+i+'</option>');
	}
	
	for(i=(currentYear-18);i>=(currentYear-77);i--)
	{

		$('.ilfsDOByear').append('<option value='+i+(i==currentYear-35?selectedOption:'')+'>'+i+'</option>');
	}
	var ilfsMonthsArr = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']
	for(i=1;i<=12;i++)
	{
		$('.ilfsDOBmonth').append('<option value='+i+(i==1?selectedOption:'')+'>'+ilfsMonthsArr[i-1]+'</option>');
	}	

	
	for(i=1;i<=31;i++)
	{
		$('.ilfsDOBday').append('<option value='+i+(i==1?selectedOption:'')+'>'+i+'</option>');
	}
    ...
    if(premURL){
        $( "#ilfsPremiumQuoteSlider" ).noUiSlider({
             range: [15, 50]
            ,start: 15
            ,step: .5
            ,connect: "lower"
            ,handles: 1
            ,slide: function(){
                var v = parseInt($('#ilfsPremiumQuoteSlider').val()).toFixed(0).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
        ...
        $( "#ilfsQuoteSlider" ).noUiSlider({
             range: [isBlineWholeLife?10000:500, isBlineWholeLife?2000000:500000]
            ,start: 100000
            ,step: isBlineWholeLife?1000:500
            ,connect: "lower"
            ,handles: 1
            ,slide: function(){
	...
	myILFS._setGAPageView(window.location.pathname+'/'+myILFS.quoteType+"-"+'life-insurance-quote-step-1','Your Details Page')	
	myILFS._setGAPageViewQuoteStarted();
}
else
{
	$('#ilfsWidget').html(window.location.host+" does not have persmission to use this quote facility.");
	// Add a pageview into google analytics
	myILFS._setGAPageView(window.location.pathname+'/'+myILFS.quoteType+"-"+'life-insurance-quote-step-permissions','No permissions to use this facility')	
...
$('#editQuoteBtn').click(function(){
	...
	// Add a pageview into google analytics
	myILFS._setGAPageView(window.location.pathname+'/life-insurance-quote-step-1','Your Details - From Quote');
	myILFS._setGAPageViewQuoteStarted();
	// hide the loading 
...
$('#backToQuoteBtn').click(function(){
    //reason for two is so that AB testing wont affect non-AB testing
    myILFS._moveScreen(5,2); // moveScreen(from, to) 
	myILFS._moveScreen(3,2); // moveScreen(from, to)
	removeStepCol("C");
	// Add a pageview into google analytics
	myILFS._setGAPageView(window.location.pathname+'/life-insurance-quote-step-2','Your Quote - From Options');	
});
...
$('#callbackBtn').click(function(){
    ...
    addStepCol('C');
    // Add a pageview into google analytics
    myILFS._setGAPageView(window.location.pathname+'/life-insurance-quote-step-3','Your Options'); 
});


$('#buyBtn, #callbackScreen').click(function(){
	myILFS._moveScreen(2,3); // moveScreen(from, to)
	addStepCol('C');
	// Add a pageview into google analytics
	myILFS._setGAPageView(window.location.pathname+'/life-insurance-quote-step-3','Your Options');	
...
$('.showEmailToSelfBreakdown ').click(function(){
    $('#emailToSelfDetails').slideDown();
    $('.showEmailToSelfBreakdown').toggle();
    $('.sendQuoteEmail').toggle();
});

$('.sendQuoteEmail ').click(function() {

    n = $('#callbackPopupName').val();
    e = $('#emailToSelfmail').val();

    if(isEmail(e) == false) {
        $(".emailToSelfMailError").show().html("Please Enter a vaild email address");
         $('.emailToSelfMailSuccess').hide().html("");
    } else {
        emailQuoteToSelf('','', e, '~', myILFS.quoteType, $("#term").val(),myILFS.quote.prem,myILFS.quote.lifeCover,'~', '~', myILFS.age1, myILFS.age2, myILFS.smokerCd1, myILFS.smokerCd2,true);
        $(".callbackPopupEmailError").hide().html("");
        $('.emailToSelfMailSuccess').show().html("Your Email has been sent");
    }
});
...
$('#getRHSCallback').click(function(){
	...
	if (z==0)
	{
		// no errors, send the callback anc change the screen
		
		//	mkAssociateLeadWithQuote('NULL', ''+$("#progressName").val(), emailAddress, ''+$("#progressPhone").val(), quoteType, numYears, prmLev,coverValue,p1gender,'NULL',(p1dobyear,p1dobmonth,p1dobday),'NULL',p1smoker,'NULL',''+$('#progressCalltime').val());
		mkAssociateLead('', n, e, p, t, myILFS.quoteTypFullName);
		$('.ilfs-right-callback').html("Thank you "+$('#callbackRHSName').val()+". Your details have been received and we will be in touch.");
		
		myILFS._setGAPageView('/life-insurance/callback-requested-RHS','Callback Requested RHS');
...
var n,e,p,t,z;
	...
	if (z==0) {
    	...
    	// Add a pageview into google analytics
    	myILFS._setGAPageView(window.location.pathname+'/life-insurance-quote-step-4','Callback Submitted');
    	myILFS._setGAPageView('/life-insurance/callback-requested-quote','Callback Requested Quote');

        mkAssociateLeadWithQuote('', n, e, p, myILFS.quoteType, $("#term").val(),myILFS.quote.prem,myILFS.quote.lifeCover,'~','~',myILFS.age1,myILFS.age2,myILFS.smokerCd1,myILFS.smokerCd2,t);
	}
});


var emailQuoteToSelf = function(firstName, lastName, emailAddress, phoneNumber, type, term, prem,sumAssured,sex1,sex2,age1,age2,smoker1,smoker2, consent){
    mkAssociateLeadPerformQuote(firstName, lastName, emailAddress, phoneNumber, type, term, prem,sumAssured,sex1,2,age1,age2,smoker1,smoker2,consent);
/*console.log(mkAssociateLeadPerformQuote(firstName, lastName, emailAddress, phoneNumber, type, term, prem,sumAssured,sex1,sex2,age1,age2,smoker1,smoker2,consent));*/
}
...
function getAnotherQuote(){
	...
	// Add a pageview into google analytics
	myILFS._setGAPageView(window.location.pathname+'/life-insurance-quote-step-1','Your Details - From Finish');
	myILFS._setGAPageViewQuoteStarted();
	// hide the loading 
	$("#getQuoteLoading").hide();
	$("#getQuote").show();
...
function getQuote(){
	...
	if (myILFS.quoteType == 'W')
	{
		if (isBlineWholeLife && myILFS.secondPerson == true)
		{
			ILFSProductID = '33';
			productLivesParam = '&productLives=3';		
		}
		else if (isBlineWholeLife && myILFS.secondPerson == false)
		{
			ILFSProductID = '23';
			productLivesParam = '&productLives=1';
		}
		else if (!isBlineWholeLife && myILFS.secondPerson == true)
		{
			ILFSProductID = '34';
			productLivesParam = '&productLives=4';
		}
		else
		{
			productLivesParam = '';
    ...
    if(premURL){
        premiumAmt = ($( "#quoteResult" ).html().replace(/,/g,'')/100)*99;
    }
	...
	if (myILFS.secondPerson==true)
	{
		if(p1smoker=="" || p2smoker==""){
			...
			if (p2dob && p1dob) {
                ...
                dataLayer.push({
                    'event':'trackEvent',
                    'category':"Quotes Data-" + myILFS.quoteType,
                    'action':"Age",
                    'label': getAge(p2dobyear,p2dobmonth,p2dobday),
                    'value':"1" 
                });
                ...
                dataLayer.push({
                    ...
                    'category':"Quotes Data-"+ myILFS.quoteType,
                    'action':"Age",
                    'label': getAge(p1dobyear,p1dobmonth,p1dobday),
                    'value': "1" 
                });

                if ( premURL) {
                     params ="quickQuoteId=lifeTermAmount"+
                        "&productId="+ILFSProductID+
                        "&coverTypeCd=L"+
                        "&jointLife=True"+
                        ...
                        "&indexation=False"+
                        "&frequencyCd=M"+
                        "&conversion=False"+productLivesParam+
				...
				ilfsQuote.loadScript('https://apps.irishlife.ie/myonlineservices/servlet/LifeCoverQuote/?'+ params, ilfsQuote.scriptLoaded,params);
				
			}
			else{
				if (termError) {
					$("#progressError").show().html("The maximum term is 40 years or up to age 80. Please reduce the term and try again.");
				}
				else {
		...
		if(p1smoker==""&& p1dob){
			...
			if (p1dob) {
                ...
                if ( premURL) {
                   ...
                   params = "quickQuoteId="+ILFSProductName+
                    "&productId="+ILFSProductID+
                    "&coverTypeCd=L"+
                    "&jointLife=False"+
                    "&dateOfBirth1Day="+p1dobday+
...
/*https://www.irishlife.ie/secure/submitLifeQuote.js?quickQuoteId=lifeTermAmount&productId=18&coverTypeCd=L&jointLife=False&dateOfBirth1Day=18&dateOfBirth1Month=5&dateOfBirth1Year=1981&sexCd1=M&smokerCd1=Y&dateOfBirth2Day=&dateOfBirth2Month=&dateOfBirth2Year=&sexCd2=M&smokerCd2=&lifeCoverAmt=0&term=5&indexation=False&frequencyCd=M&conversion=False&premiumAmt=15*/

/*http://www.irishlife.ie/servlet/submitLifeCoverCalcQuote.do?quickQuoteId=lifeTermAmount&productId=18&coverTypeCd=L&jointLife=False&dateOfBirth1Day=1&dateOfBirth1Month=1&dateOfBirth1Year=1973&sexCd1=M&smokerCd1=N&lifeCoverAmt=0&term=10&indexation=False&frequencyCd=M&conversion=False&premiumAmt=36.63*/
...
var script = document.createElement("script")
	...
	var isBlineWholeLife = (window.location.href.indexOf("bline") > -1 && window.location.href.indexOf("whole-") > -1);
		...
		if (v !="error"){ // success
			...
			myILFS.cover = (parseInt(r.lifeCover*100)/100).toFixed(0);
			$('.ilfsQuoteResult').html(" €" + myILFS.cover.replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + " costing <br /> €"+(parseInt(myILFS.quote.premInclLevy*100)/100).toFixed(2));
			if (myILFS.secondPerson == true && ILFSDualPersons == false) {
				if(ILFSDualPersons){
                    ...
                    var policyCover = (parseInt(myILFS.quote.fee*100)/100).toFixed(2);
                    var jointPremium = (parseInt((myILFS.quote.prem)*100)/100).toFixed(2);
					$('.quoteBreakdownDetails1').html("Cost of Life Cover: €"+(jointPremium - policyCover));
				}
            ...
            if (premURL){
                $('.progressLifeCoverAmount').html("€"+(parseInt(r.lifeCover*100)/100).toFixed(0).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,"));
            }
            else{
            ...
            //Google analytics
             try {
                dataLayer.push({
                    ...
                    'category':"Quotes Data-"+ myILFS.quoteType,
                    'action':"Premium",
                    'label':(parseInt(myILFS.quote.premInclLevy*100)/100).toFixed(2),
                    'value':"1"
                });
                ...
                dataLayer.push({
                    ...
                    'category':"Quotes Data-"+ myILFS.quoteType,
                    'action':"Sum Assured",
                    'label':(parseInt(r.lifeCover*100)/100).toFixed(0).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,"),
                    'value':"1"
                });
			...
			if (!isNaN(r.term)) {
				$('.progressLifeCoverYears').html("for " + r.term + " years");
			}
			...
			//bline wol quote charts
			if (isBlineWholeLife)
			{
				youngest = r.age1;
				if (myILFS.secondPerson)
				{
					youngest = (r.age1 < r.age2)?r.age1:r.age2;
				}
				drawCostSumChart((parseInt(myILFS.quote.premInclLevy*100)/100).toFixed(2),(parseInt(r.lifeCover*100)/100).toFixed(0));
				drawRateReturnChart((parseInt(myILFS.quote.premInclLevy*100)/100).toFixed(2),
								(parseInt(r.lifeCover*100)/100).toFixed(0),								
								youngest);
			}
			...
			*/	
			...
			var options=['3','4','5'];
			var weight=[0.2,0.3,0.5];
			var whichOption = getRandomItem(options, weight);
			
            if (myILFS.quoteType == 'M'){ //advisor                
				$("#detailsABOption"+whichOption).show();
            } else if (r.age1 > 49 && myILFS.quoteType != 'M'){ //over 50s option
                $("#detailsABOption2").show();
            } else if (r.age1 > 18 && r.term < 21 && parseInt(myILFS.cover) <= 150000){ //simple option
                $("#detailsABOption1").show();
            } else {
                $("#detailsABOption"+whichOption).show();
            }
			...
			myILFS._setGAPageView(window.location.pathname+'/life-insurance-quote-step-2','Your Quote');
			myILFS._setGAPageViewQuoteCompleted();
			
		}
		else {
			// Add a pageview into google analytics
			myILFS._setGAPageView(window.location.pathname+'/life-insurance-quote-step-1','Error Getting Quote')
