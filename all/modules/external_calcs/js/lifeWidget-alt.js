...
//	stylesheet.href = 'http://ildev.internal.irishlife.ie:8080/uploadedFiles/retail/123/css/ilfsWidget.css';
	...
	html.detailsUserA = '<div class="sixcol"><div id="detailsUserA" class="ilfsDetailsPanel"><div class="ilfsDetailsBlock"><h3>Person 1</h3><label>Your Name</label><input  class="text-input" type="text" name="quoteName" id="quoteName" /><div class="clearer"></div><label>Email Address</label><input type="email" class="text-input" name="quoteEmail" id="quoteEmail" /><div class="clearer"></div><label>Phone Number</label>	<input  class="text-input" type="number" name="quotePhone" id="quotePhone" /><div class="clearer"></div><label>Are you a smoker?</label><select id="ilfsUserA-smoker" ga-change ga-category="ilfs-life-quote" ga-action="Page 1" ga-label="smoker-1 dropdown clicked"><option value="Y">Yes</option><option value="N" selected="selected">No</option></select><div class="clearer"></div><label>Date of Birth</label>'+html.dobA+'</div></div></div>';
	...
	html.screen1ComplyBox = '<strong>We are asking for your email and phone number so that we may contact you regarding your quote. As part of this application we will email you your quote. We will <i>not</i> use your information for any other purpose. <br>This indicative quotation is for a '+ILFSProductScreenProductName+' protection policy which is provided by Irish Life Assurance plc.</strong> You may have other protection needs and we always recommend that you should discuss your overall insurance needs with a Financial Advisor.</div>';
...
'<tr style="display:none">\
    <td id ="emailToSelfContent" class="content">\
...
$('.sendQuoteEmail ').click(function() {
    ...
    if(isEmail(e) == false) {
        ...
        emailQuoteToSelf('','', e, '~', myILFS.quoteType, $("#term").val(),myILFS.quote.premInclLevy,myILFS.quote.lifeCover,'~', '~', myILFS.age1, myILFS.age2, myILFS.smokerCd1, myILFS.smokerCd2,true);
...
var n,e,p,t,z;
	...
	if (z==0) {
        ...
        mkAssociateLeadWithQuote('', n, e, p, myILFS.quoteType, $("#term").val(),myILFS.quote.premInclLevy,myILFS.quote.lifeCover,'~','~',myILFS.age1,myILFS.age2,myILFS.smokerCd1,myILFS.smokerCd2,t);
	}
});
...
var emailQuoteToSelf = function(firstName, lastName, emailAddress, phoneNumber, type, term, prem,sumAssured,sex1,sex2,age1,age2,smoker1,smoker2, consent){
	var hash = getHash(key+emailAddress);
			...
			FirstName: firstName,MobilePhone: phoneNumber,LastName: lastName, Callbackrequested: true,LeadSourceDetail:'Quote with contact details supplied',
			QuoteLifeTermAge1: age1,QuoteLifeTermAge2:age2,QuoteLifeTermPremium:parseFloat(Math.round(prem * 100) / 100).toFixed(2),
			QuoteLifeTermSmoker1:smoker1,QuoteLifeTermSmoker2: smoker2, 'Permissions-Email':false,
	LastGACampaignNameVisit:gaCampaign,
	OriginalGACampaignVisit:gaCampaign,
	LastGAMedium: gaMedium,
	OriginalGAMedium: gaMedium,	
			QuoteLifeTermSex1:sex1,QuoteLifeTermSex2:sex2,QuoteLifeTermSumAssured:sumAssured,QuoteLifeTermTerm:term,QuoteType:'T'},hash);	
...
function getQuote(){
	...
	$('#callbackPopupName').val($('#quoteName').val());
	$('#callbackPopupPhone').val($('#quotePhone').val());
	$('#callbackPopupEmail').val($('#quoteEmail').val());
	...
	else if (!isEmail($('#quoteEmail').val()))
	{
		alert("Please enter a valid email address ");
		validQuote = false;
	}
	else if ($('#quotePhone').val() == '')
	{
    ...
    scriptLoaded: function(params) {
		...
		if (v !="error"){ // success
			...
			*/	
			...
			emailQuoteToSelf('',$('#callbackPopupName').val(), $('#callbackPopupEmail').val(), $('#callbackPopupPhone').val(), myILFS.quoteType, $("#term").val(),myILFS.quote.premInclLevy,myILFS.quote.lifeCover,'~', '~', myILFS.age1, myILFS.age2, myILFS.smokerCd1, myILFS.smokerCd2,true);
			$("#getQuoteLoading").hide();
