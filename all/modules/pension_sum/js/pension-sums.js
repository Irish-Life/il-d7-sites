(function ($) {
  ...
  for (i = 0; i < capFactors.length; ++i) { capFactors[i] = new Array(); }
  ...
  statePension[0] = 12131; //11976
  statePension[1] = 22999; //22703
  ...
  $(document).ready(function () {
    ...
    $('.pensionCTEuroAmountFeeResult').change(function () {
      ...
      $('.pensionCTTakeupResult').html(ans + '' + $('.pensionCTEuroAmountFeeResult').val());
    });
    ...
    for (i = 0; i < productnames.length; i++) {
      $('#product').append('<option value="' + productnames[i] + '">' + productnames[i] + '</option>');
    }
    ...
    $("#ps-advice-fee-com-block input:radio").click(function () {


      feeCommission = $("input:radio[name=ps-advice-fee-comm]:checked").val();
      if (feeCommission == "fee") {
        $('#commission-choice-percent').hide();
        $('#commission-choice-cash').show();
      } else {

        $('#commission-choice-percent').show();
        $('#commission-choice-cash').hide();
  ...
  doCalcs = function (n) {
    ...
    termNRA = nra - age - 1;
    servicenum = (Number(service) ? service : "1");
    serviceNRA = Number(termNRA) + Number(servicenum);
    finalIncome = Number(sal * Math.pow(1.03, termNRA));

    gender = Number($("input:radio[name=ps-gender]:checked").val());

    marital = Number($("input:radio[name=ps-marital]:checked").val());


    incomeTaxAim = (Number($('.rateAim').val()) ? Number($('.rateAim').val()) : 0);
    ...
    if (marital == "1") {
      $('.maritalResult').html("Married");
    } else {
      $('.maritalResult').html("Single");
    }
    ...
    if (whichOne == 'B') {
      calcLSPremGet(age, sal, nra, maxLumpSum1, maxLumpSum2, prevLumpSum, currentValue);
    }
    ...
    //A Pension Gap
    log('incomeTaxAim ' + incomeTaxAim + ' marital ' + marital);
    ...
    var incomeGap = 0;
    if (whichOne == 'A') {
      //log('age: '+age+' sal: ' +sal+' nra: ' +nra+' serviceNRA: ' +serviceNRA+' ' +Number(currentValue)+Number(currentValuePrevJobs)+' ' +marital+' ' +$('.pensionCTSliderSpouseResult').html()+' taxThreshold ' + taxThreshold)
    ...
    setGAPageView('/pensions/pension-sums/rev-max', 'Rev Max Calulator');
  ...
  function calcLSPremGet(age, income, nra, lumpsum1, lumpsum2, prevLumpSum, currentValue) {

    //log(age+" "+ income+" "+nra+" "+lumpsum1+" "+lumpsum2+" "+prevLumpSum+" "+currentValue);
    var prem1 = 0;
    var prem2 = 0;
    var today1 = 0;
    var today2 = 0;

    $.get("https://apps.irishlife.ie/myonlineservices/servlet/submitPensionQuote?type=1&freq=Y&age=" + age + "&income=" + income + "&ret=" + nra + "&target1=" + lumpsum1.toFixed(2) + "&target2=" + lumpsum2.toFixed(2) + "&sp=" + currentValue.toFixed(2), function (data) {
  ...
  function calcRevMaxProjections(age, income, nra, ap1, sp1, ap2, sp2, ap3, sp3) {
    ...
    $.get("https://apps.irishlife.ie/myonlineservices/servlet/submitPensionQuote?type=3&freq=M&age=" + age + "&income=" + income + "&ret=" + nra + "&ap1=" + ap1 + "&sp1=" + sp1 + "&ap2=" + ap2 + "&sp2=" + sp2 + "&ap3=" + ap3 + "&sp3=" + sp3, function (data) {

      str = jQuery.trim(data);
      if (str.length > 10) {
        var variables = str.split('&');
        value1 = Number(variables[0].substr(13, variables[0].length).split('=')[1]);
        today1 = Number(variables[1].split('=')[1]);
        value2 = Number(variables[2].split('=')[1]);
        today2 = Number(variables[3].split('=')[1]);
        value3 = Number(variables[4].split('=')[1]);
        today3 = Number(variables[5].substr(0, variables[5].length-2).split('=')[1]);
      }
  ...
  function calcIncomeGap(age, income, nra, target, marital, serviceNRA, spouse) {
    ...
    $.get("https://apps.irishlife.ie/myonlineservices/servlet/submitPensionQuote?type=4&freq=Y&age=" + age + "&income=" + income + "&ret=" + nra + "&target=" + target + "&serviceNRA=" + serviceNRA + "&marital=" + marital + "&spousePension=" + spouse, function (data) {
  ...
  function projectCurrentPension(age, income, nra, serviceNRA, sp, marital, spouse, taxThreshold) {
    ...
    $.get("https://apps.irishlife.ie/myonlineservices/servlet/submitPensionQuote?type=2&freq=Y&age=" + age + "&income=" + income + "&ret=" + nra + "&sp=" + sp + "&serviceNRA=" + serviceNRA + "&marital=" + marital + "&spousePension=" + spouse, function (data) {
  ...
  loadSliders = function () {
    ...
    $(".pensionCTSliderYearsCurrentEmployer").slider({
      handle: '#slider-handle', range: "min", animate: true, value: yearsStart, min: 0, max: 40, step: 1,
    ...
    $(".pensionCTPercentAmountFee").slider({
      ...
      slide: function (event, ui) {
        $('.sliderBubble').fadeOut('fast');
        $(".pensionCTPercentAmountFeeResult").html(ui.value);
      },
      start: function (e, ui) {
    ...
    $(".pensionCTSliderSPorTV").slider({
      handle: '#slider-handle', range: "min", animate: true, value: earnStart, min: 10000, max: 500000, step: 1000,
      slide: function (event, ui) {
  ...
  updateCommissionInputs = function () {
    ...
    var premiumBand;
    var sportv = $(".pensionCTSliderSPorTVResult").val();
    if (product == productnames[2] || product == productnames[3] || product == productnames[4]) {
      // alert('pensionCTSliderSPorTVResult: ' + $( ".pensionCTSliderSPorTVResult" ).val());
  ...
  scrollToHead = function () {
    ...
    $('html, body').animate({
      scrollTop: $('#ps-top').offset().top - 25
    }, 500);
  }
  $.fn.digits = function () {
