...
var bubblesExist=false;
	...
	d3.loadData()
        ...
        .csv('stats', '/uploadedFiles/retail/js/ilfs/claimStats/stats-2013.csv')
	...
	var masterSwitch = d3.select("#buttonHolder").selectAll(".master")
		...
		.on("mouseover", function(d,i){
			...
			if(master!==drop)d3.select("#master_"+drop).transition().duration(500).style("top","36px");
			}
		})
		.on("click", function(d,i){
			master = masterToggle(i);
			selectMaster(master);
			if(!touch){d3.selectAll(".master").transition().duration(500).style("top","0px");}
	...
	if(touch==true){d3.select("#master_SIC").style("top","36px");}
