...
getChartDataAgain = function(){
	...
	$('#fundsChartChart').html("<h2>LOADING...</h2><div class=\"progress supplementary\"><span class=\"meter\" style=\"width:0%\"></span></div>");
		name = name.split(';')[1];
			...
			$.getJSON('/servlet/fundpricejson?fund='+fundSelected.split(';')[0], function (data, error){
...
(function($){
    ...
    $( document ).ready(function() {
        ...
        $.get('/servlet/jsonFund.jsp?fund='+fundSelected.split(';')[0]+'&page=prices', function (data, error){})
    	.done(function ( data ) {

            /*Tidy up response*/
            //console.log("url = " + fundSelected.split(';')[0]);
            //remove whitespace from before and after data
            var nospaces = data.replace(/^\s\s*/, '').replace(/\s\s*$/, '');
            //remove whitespace from before and after data
            ...
            var priceDate =  fundData[0].funds[0].priceDate;
            var fundCharge = parseFloat(fundData[0].funds[0].fmc);
            var latestPrice = parseFloat(fundData[0].funds[0].latest);
            var threeMonthGrowth = parseFloat(fundData[0].funds[0].threeMonthGrowth);
            var sixMonthGrowth = parseFloat(fundData[0].funds[0].sixMonthGrowth);
            var last12Months = parseFloat(fundData[0].funds[0].last12Months);
            var last3Years = parseFloat(fundData[0].funds[0].last3Years);
            var last5years = parseFloat(fundData[0].funds[0].last5years);
            var sinceLaunchGrowth = parseFloat(fundData[0].funds[0].sinceLaunchGrowth);
