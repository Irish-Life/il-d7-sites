...
var bubblesExist=false;
	...
	d3.loadData()
        .json('counties', '/sites/all/modules/claim_stats_map/js/ireland.js')
        .csv('stats', statCSV)
	...
	var masterSwitch = d3.select("#buttonHolder").selectAll(".master")
		...
		.on("mouseover", function(d,i){
			...
			if(master!==drop)d3.select("#master_"+drop).transition().duration(500).style("top","+100px");
			}
		})
		.on("click", function(d,i){
			master = masterToggle(i);
			selectMaster(master);
			if(!touch){d3.selectAll(".master").transition().duration(500).style("top","+64px");}
	...
	if(touch==true){d3.select("#master_SIC").style("top","-64px");}
