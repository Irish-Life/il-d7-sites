...
//		console.log("updateSearchResultsContact searchCountFull "+searchCountFull);
...
*/
	...
	updateSearchResultsAccounts = function(myBrokers){
		...
		$('.search-loading').html('<i class="fa fa-clock-o" aria-hidden="true"></i>&nbsp;Still searching, nearly there...');
			...
			// check if the cleaned account search results have anyting in them and if so then display
			if(clean.length>0){						
		...
		performSearch = function(){
			...
			if (searchq.length>=3){
				...
				//console.log('myBrokersDup '+JSON.stringify(myBrokersDup));
				//console.log('myBrokers '+JSON.stringify(myBrokers));
			}else{
				$('.search-error').show();
			}
		}
