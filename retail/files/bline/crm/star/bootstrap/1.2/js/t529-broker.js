(function($) {
	...
	var showBrokerActivity = function(r){
		...
		if (brokerActivities.length <=0){
			...
			for(var i = 0; i<brokerActivities.length; i++)	{
				...
				//console.log('icon '+icon+' '+JSON.stringify(sourceofcontact)+'\n ');
				if(sourceofcontact.length>0){
				resultHTML+='<tr>'+
				  '<td>'+getActivityIcon('astask')+'</td>'+
				  '<td><a href="javascript:void(0);" class="open'+icon+'" data-activityid="'+activityid+'">('+sourceofcontact+') '+subject+'</td>'+
				  '<td>'+date+'</td>'+
	...
	function getActivityIcon(activityValue){
		...
		if (activityValue=="phonecall"){
			...
			activityIcon = '<i style="color:#9D46B7;" class="fa fa-volume-control-phone" data-toggle="tooltip" data-placement="top" title="Advisory Services Task"></i>';
		}
		return activityIcon;
	}
