...
// case 2
if($("#about-age").val() >= 60 && $("#about-age").val() < 65)	{
   maxCircle=2;
...
var widowsPension = 960.77;
var widowsPensionWithChild = 150;
...
function calcTerm() {
	...
	//if children yes & mortgage yes
	if ($("#radio3").is(":checked") && $("#income-age").val() > 0) {
			term = 25 - childAge;
			//console.log('The term is ' + term);
			
		}
		
	//if children no & mortgage yes	
	else if ($("#radio4").is(":checked") && $("#income-age").val() > 0) {
			 term = $("#income-age").val();
			//console.log('The term is ' + term + ' years');
...
function calculateStateBenefit() {
	...
	//children?	
	stateBenefit += widowsPensionWithChild*Number($('#childNum').val());
	return stateBenefit;
};	
	

//Calculate the lumpsum on death
function calcDeath(netIncome, existingLifeCover, insertDeathValue, insertIncomeGap, insertMortgage, hideInfo, fadeInMessage, hideChart, termVal) {
	
	//married = state benefit, not married = no state benefit
	var stateBenefit = calculateStateBenefit();
	
	
	//life insurance need if 
	// - have children, and or,
	// - mortgage
				
	
	//if married yes & children yes & mortgage yes
	//if (($('#about-rel option:selected').val() === '1') && $("#income-age option:selected").val() > 0) {
	 
	 //console.log('Conditions right for lumpsum calc ' + termVal);
	 
	 var sumDeath = 0;	
	 
	 var monthlyGap = Number(removeCommaFromNumber(netIncome)) - stateBenefit - Number(removeCommaFromNumber($('.output2').html()));
	 var overallTermGap = monthlyGap * 12 * Number(termVal);
	 
	  //if not married and have no children lumpum on death is 0 
	 if($('#about-rel option:selected').val() === '2' || $('#partner-rel option:selected').val() === '2' && $('#radio4').is(':checked')) {
		 sumDeath;
...
//calculate bill pay 
function calcBillPay(existingIncomeProtection,insertBillPayValue, hideInfo, fadeInMessage, whichAge, whichOne, isIP) {
	
	var billPaySum;
	
	if (isIP && $('#phiOccClass'+whichOne).html() != 'N' && !existingIncomeProtection && $(whichAge+'age').val() <= 60) 
	{
		billPaySum = Math.round(Number($('.range2').val()) + 300);
	}
	...
	$('#billPayMortgage').html('€' + $('.output2').val());
	
	if (billPaySum <= 0) {
		$('#billPayMortgage').html('');
		hideInfo.addClass('hidden');
		fadeInMessage.removeClass('hidden');
		$('.billpay-needs').addClass('hidden');
	}
	
	else {
		hideInfo.removeClass('hidden');
		fadeInMessage.addClass('hidden');
		$('.billpay-needs').removeClass('hidden');
		$('#incomeIllBill').html('We will provide €' + billPaySum);
	}
...
function calcSpecifiedIll(netIncome,existingSpecIllCover,insertSpecIllValue,insertSalary,salary,hideInfo,fadeInMessage,hideChart,whichAge,whichOne) {
	...
	if($('#occupationClass'+whichOne).html() == 'N') {
...
$( ".result" ).click(function() {
	...
	var billPayCover2=800;		
	var termVal = calcTerm();
	
	
	//console.log('Calculate for one person');
	lifeCover1=calcDeath($('.output1').html(), $('.output3').html(), $('#lumpsumDeath'), $('.income-gap'), $('#insert-mortgage'), $('.you-info-death'), $('.ifcovered-message'), $('.your-needs-death'), termVal);
	sicCover1=calcSpecifiedIll($('.output1').html(), $('.output4').html(), $('#specIll'),$('#salary'),$('.output1').html(),$('.you-info-specIll'), $('.ifcovered-message3'), $('.your-needs-specIll'),'#about-',1);
	billPayCover1=calcBillPay($('#radio11').is(':checked'),$("#billPayCover1Amt1"), $('.you-info-billpay'), $('.ifcovered-message2'),'#about-',1,isBrokerVersion);
	//console.log("Your Bill Pay cover is " + billPayCover1);

	
	//this checks if both you and partner have given an income value
	if ($('.range1').val() > 1000 && $('.range5').val() > 1000 && !$("#partner-existing-fields").hasClass('hidden')) {
		//console.log('Calculate for two people');
		lifeCover2=calcDeath($('.output5').html(), $('.output6').html(), $('#lumpsumDeathPartner'), $('.income-gap-partner'), $('#insert-mortgage-partner'), $('.partner-info-death'), $('.ifcovered-message-partner'),$('.partner-needs-death'), termVal);
		sicCover2=calcSpecifiedIll($('.output5').html(), $('.output7').html(), $('#specIllPartner'),$('#salary-partner'),$('.output5').html(),$('.partner-info-specIll'), $('.ifcovered-message-partner3'),$('.partner-needs-specIll'),'#partner-',2);
		billPayCover2=calcBillPay($('#radio9').is(':checked'),$("#billPayCover1Amt2"),$('.partner-info-billpay'), $('.ifcovered-message-partner2'),'#partner-', 2,isBrokerVersion);
	...
	if (isBrokerVersion)
	{
		...
		if (lifeCover1 > 0 || lifeCover2 > 0) getTheBrokerQuote('L',twoPeople, lifeCover1, lifeCover2, termVal,1);
		if (sicCover1 > 0 || sicCover2 > 0) getTheBrokerQuote('LSI',twoPeople, sicCover1, sicCover2, termVal,2);
		if (billPayCover1 > 0)
		{
	...
	else
	{
		...
		getTheQuote(twoPeople, lifeCover1, sicCover1, billPayCover1, lifeCover2, sicCover2, billPayCover2, termVal);
	}
...
function getTheQuote(twoPeople, lifeCover1Amt, sicCover1Amt, billPayCover1Amt, lifeCover2Amt, sicCover2Amt, billPayCover2Amt, termVal)
{
	var headID = document.getElementsByTagName("head")[0];
	var newScript = document.createElement('script');
	var params;
	...
	if (lifeCover1Amt > 0)
	{
		lifeCover1="lifeCoverAmt1="+lifeCover1Amt+"&lifeCoverTerm1="+termVal+"&lifeCoverConversion1=Y";
		params += "&" + lifeCover1;
	}
	
	if (sicCover1Amt > 0)
	{
		sicCover1="specifiedIllnessAmt1="+sicCover1Amt+"&specifiedIllnessTerm1="+termVal+"&specifiedIllnessConversion1=Y";
		params += "&" + sicCover1;
	}
	
	if (billPayCover1Amt > 0 && $('#occupationClass1').html() != 'N')
	{
		billPay1="billPayAmt1="+billPayCover1Amt+"&billPayTerm1="+termVal+"&billPayDeferredWeeks1=8&billPayOccupationClassCd1="+
		$('#occupationClass1').html() + "&billPayConversion1=Y&billPayExpiration1=5";
		params += "&" + billPay1;
	}
	...
	if (twoPeople)
	{
		...
		if (lifeCover2Amt > 0)
		{
			lifeCover2="lifeCoverAmt2="+lifeCover2Amt+"&lifeCoverTerm2="+termVal+"&lifeCoverConversion2=Y";
			params += "&" + lifeCover2;
		}
	
		if (sicCover2Amt > 0)
		{
			sicCover2="specifiedIllnessAmt2="+sicCover2Amt+"&specifiedIllnessTerm2="+termVal+"&specifiedIllnessConversion2=Y";
			params += "&" + sicCover2;
		}
		
		if (billPayCover2Amt > 0 && $('#occupationClass2').html() != 'N')
		{
			billPay2="billPayAmt2="+billPayCover2Amt+"&billPayTerm2="+termVal+"&billPayDeferredWeeks2=8&billPayOccupationClassCd2=" + 
			$('#occupationClass2').html() + 
			"&billPayConversion2=Y&billPayExpiration2=5";
			params += "&" + billPay2;
		}
	}
